...
while (significant==T) { # While loop to continue add columns to X until the coefficient are not statistically significant
 ...
 R2_SG<-R2_SG[order(R2_SG[,2],decreasing =T),] # After building the model with all i, we sort the the R2 matrix by their adjusted R2
...
while (significant==T) {
 ...
 R2_GS<-R2_GS[order(R2_GS[,2],decreasing =T),] # After building the model with all i, we sort the the R2 matrix by their adjusted R2
